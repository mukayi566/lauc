...
const FinanceLogin = () => {
    ...
    const getFirebaseErrorMessage = (code) => {
        switch (code) {
            case 'auth/user-not-found': return 'Account not found in our financial records.';
            case 'auth/wrong-password': return 'Incorrect PIN/Password.';
            case 'auth/invalid-credential': return 'Authentication failed. Please verify your email and password.';
            case 'auth/invalid-email': return 'Please enter a valid institution email.';
            case 'auth/user-disabled': return 'This financial account has been suspended.';
            case 'auth/too-many-requests': return 'Suspicious activity detected. Please try again later.';
    ...
    const handleResetPassword = async (e) => {
        ...
        try {
            ...
            setSuccessMsg('Security reset link sent to your registered email!');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (role !== 'finance') {
                const msg = 'Access Denied: Restricted to Finance Department personnel only.';
                setError(msg); toast.error(msg); await auth.signOut(); setLoading(false); return;
            }

            toast.success('Financial Officer Authenticated');
    ...
    return (
        <div style={{ minHeight: '100vh', display: 'flex', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
            <div
                ...
                style={{
                    flex: '0 0 42%',
                    background: 'linear-gradient(160deg, #065f46 0%, #064e3b 50%, #065f46 100%)',
                    display: 'flex',
            ...
            >
                ...
                <div style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
                    <div style={{ width: 100, height: 100, borderRadius: '50%', background: 'rgba(255, 255, 255, 1)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px', fontSize: 36, color: '#064e3b', backdropFilter: 'blur(8px)', boxShadow: '0 8px 32px rgba(0,0,0,0.1)' }}>
                        <img src={logo} alt="Logo" style={{ width: 105 }} />
                    </div>
                    <h1 style={{ color: 'white', fontWeight: 800, fontSize: 26, marginBottom: 10, letterSpacing: '-0.02em' }}>Finance Department</h1>
                    <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: 14, lineHeight: 1.8, maxWidth: 280 }}>
                        Student payment management, transaction tracking and financial reporting systems.
                    </p>

                    <div style={{ marginTop: 36, display: 'flex', flexDirection: 'column', gap: 12 }}>
                        {['Fee Management', 'Transaction Verification', 'Financial Reports', 'Salary Administration'].map(f => (
                            <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'rgba(255,255,255,0.85)', fontSize: 13 }}>
                                <span style={{ width: 22, height: 22, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                    <i className="fas fa-wallet" style={{ fontSize: 10, color: 'white' }}></i>
                                </span>
                                {f}
                            </div>
                        ))}
                    ...
                    {view === 'login' ? (
                        <>
                            <div style={{ marginBottom: 32 }}>
                                <h2 style={{ fontSize: 26, fontWeight: 800, color: '#064e3b', marginBottom: 6 }}>Officer Sign In</h2>
                                <p style={{ color: '#64748b', fontSize: 14 }}>Access the financial management console</p>
                            </div>
                            ...
                            <form onSubmit={handleSubmit}>
                                <div style={{ marginBottom: 18 }}>
                                    <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 6 }}>Corporate Email</label>
                                    <div style={{ position: 'relative' }}>
                                        <i className="fas fa-user-shield" style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: '#059669', fontSize: 14 }}></i>
                                        <input
                                            type="email"
                                            name="email"
                                            value={form.email}
                                            ...
                                            required
                                            placeholder="finance@fairview.edu"
                                            style={inputStyle}
                                        />
                                    </div>
                                </div>
                                ...
                                <div style={{ marginBottom: 8 }}>
                                    <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 6 }}>Secure Password</label>
                                    <div style={{ position: 'relative' }}>
                        ...
                        <>
                            <div style={{ marginBottom: 32 }}>
                                <h2 style={{ fontSize: 26, fontWeight: 800, color: '#064e3b', marginBottom: 6 }}>Security Reset</h2>
                                <p style={{ color: '#64748b', fontSize: 14 }}>Request a system-generated password reset link.</p>
                            </div>
